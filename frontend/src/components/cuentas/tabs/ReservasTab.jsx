...
export function ReservasTab({ cuentaId }) {
  ...
  const fetchOrders = useCallback(async (pg = 1) => {
    setLoading(true);
    try {
      const r = await api.get(`/cuentas/${cuentaId}/ventas/orders`, { params: { doc_tipo: "RESERVA", page: pg, limit: 50 } });
  ...
  const fetchLines = useCallback(async (pg = 1) => {
    ...
    try {
      const r = await api.get(`/cuentas/${cuentaId}/ventas/lines`, { params: { doc_tipo: "RESERVA", page: pg, limit: 50 } });
