...
import {
  Loader2, Phone, MessageCircle, MapPin, StickyNote, Mail,
  DollarSign, ShoppingCart, RefreshCw, TrendingUp, AlertOctagon, PhoneIncoming,
  CalendarClock, Pencil, ChevronDown, ChevronUp, ScrollText,
  Check, X as XIcon, Minus, PhoneOff,
} from "lucide-react";
...
// ─── CRM-D12: Historial reciente de interacciones ────────────────────
// Mini-tablas de tipos/outcomes (subset solo para visualización).
// No reusamos las de NuevaInteraccionModal para evitar acoplar 2 modales.
// Si aparece un valor legacy fuera de esta tabla, usamos fallback gris.
...
function HistorialInteracciones({ partnerOdooId, disabled }) {
  ...
  useEffect(() => {
    ...
    let cancel = false;
    setLoading(true);
    api.get(`/cuentas/${partnerOdooId}/interacciones`, { params: { limit: 3 } })
  ...
  // Ocultar la sección completa si no hay interacciones (regla del spec)
  if (loading) {
    return (
            ...
            return (
              <div key={it.id} className="flex gap-2 text-[12px]" data-testid={`hist-item-${it.id}`}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div
                    className="text-slate-600 italic truncate mt-0.5"
                    title={resumen}
                  >
                    "{resumenCorto}"
                  </div>
